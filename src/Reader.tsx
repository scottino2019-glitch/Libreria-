...
export const ReaderView = ({ book, onClose }: ReaderProps) => {
  ...
  const autoFitScale = useCallback(async (loadedPdf: pdfjsLib.PDFDocumentProxy) => {
    ...
    try {
      ...
      const containerWidth = containerRef.current.clientWidth - (window.innerWidth < 1024 ? 32 : 80);
      
      // Aim for a comfortable reading width
      const targetWidth = Math.min(containerWidth, 900);
      let newScale = targetWidth / viewport.width;
      
      // On mobile or small screens, ensure pages aren't minuscule.
      // E.g., we set a minimum scale of 1.25 so letters are actually clear and readable.
      // This supports standard horizontal scrolling or zooming for absolute comfort.
      if (window.innerWidth < 640) {
        newScale = Math.max(newScale, 1.25);
      } else {
  ...
  const loadPdf = useCallback(async () => {
    try {
      ...
      if (book.file) {
        ...
        if (!response.ok) {
          throw new Error('Impossibile scaricare il volume dal server della biblioteca.');
        }
        arrayBuffer = await response.arrayBuffer();
      } else {
        throw new Error('Nessun file o indirizzo URL disponibile per questo volume.');
      }
  ...
  }, [book.file, book.url, autoFitScale]);

  useEffect(() => {
    loadPdf();
  }, [loadPdf]);

  // Track scroll to update current page number and persist it
  useEffect(() => {
  ...
  }, [pdf, currentPage, book.id]);

  // Scroll to last read page on mount
  useEffect(() => {
    if (pdf && book.lastPage && book.lastPage > 1) {
      ...
      <div className="flex-1 w-full flex overflow-hidden relative">
        ...
        >
          {error ? (
            ...
            <div className="py-4 sm:py-8 min-w-max w-full">
        ...
        {/* Floating Zoom Controls at Bottom Right - Always accessible, does not wrap on small screens */}
        {!isLoading && !error && pdf && (
          <div className="fixed sm:absolute bottom-6 right-6 bg-[#211b15] border border-gold/40 text-[#d4c4b5] rounded-full px-4 py-2 flex items-center gap-3 shadow-2xl z-[150] active:scale-100 transition-transform">
            <button 
              onClick={() => setScale(s => Math.max(0.5, Number((s - 0.15).toFixed(2))))} 
              className="p-1.5 hover:text-gold active:scale-90 transition-all"
              title="Riduci zoom"
            >
              <ZoomOut size={18} />
            </button>
            <span className="text-xs font-serif min-w-[3rem] text-center font-bold tracking-wider select-none text-gold">
            ...
            <button 
              onClick={() => autoFitScale(pdf)}
              className="px-2 py-0.5 text-[10px] uppercase font-sans tracking-wide border border-gold/20 rounded bg-gold/10 hover:bg-gold hover:text-white transition-colors cursor-pointer text-xs font-semibold"
